...
LeafNode::LeafNode(FPTree* t) {
    ...
    stringstream ss;
    ss << pPointer.fileId;
    string s_pPointer_fileId;
    ss >> s_pPointer_fileId;

    filePath = DATA_DIR + s_pPointer_fileId;
...
bool FPTree::bulkLoading() {
    ...
    queue<Node *> startLeafs;
    size_t m = 0, n = 0;
    while(startLeaf){
        startLeafs.push(startLeaf);
        ++m;
        startLeaf = startLeaf->next;
    }
    while (startLeafs.size() > 1 || startLeafs.front()->ifLeaf()) {
        InnerNode* node = new InnerNode(degree, this);
        size_t size = m < 2 * degree + 1 ? m : degree;
        for (size_t i = 0; i < size; ++i) {
            InnerNode *temp = (InnerNode *)startLeafs.front();
            node->insertLeaf(KeyNode{temp->keys[0], temp});
            startLeafs.pop();
            --m;
        }
        startLeafs.push(node);
        ++n;
        if (m == 0){
            m = n;
            n = 0;
    ...
    this->root = (InnerNode *)startLeafs.front();
