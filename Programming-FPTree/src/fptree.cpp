...
void InnerNode::insertNonFull(const Key& k, Node* const& node) {
	...
	memmove(keys + index + 1, keys + index, sizeof(Key) * (nKeys - index));
	memmove(childrens + index + 2, childrens + index + 1, sizeof(Node *) * (nChild - index - 1));
	keys[index] = k;
...
KeyNode InnerNode::insert(const Key& k, const Value& v) {
    ...
    // TODO
	newChild = this->childrens[findIndex(k)]->insert(k, v);
	if(newChild.node != nullptr){
...
KeyNode InnerNode::insertLeaf(const KeyNode& leaf) {
	...
	if(this->childrens[0]->ifLeaf() == false){
		newChild = dynamic_cast<InnerNode *>(this->childrens[findIndex(leaf.key)])->insertLeaf(leaf);
		if(newChild.node != nullptr){
			if(this->nKeys >= 2 * degree){
				insertNonFull(newChild.key, newChild.node);
				newChild = split();
...
bool InnerNode::remove(const Key& k, const int& index, InnerNode* const& parent, bool &ifDelete) {
    ...
    if(ifDelete == true){
        ...
        if(keyIndex == 0){
            ...
            this->removeChild(keyIndex - 1, keyIndex);
        }
        // only have one leaf
        // TODO
...
void InnerNode::removeChild(const int& keyIdx, const int& childIdx) {
    //cout << "This node childNum: " << this->nChild << ", keyNum: " << this->nKeys << endl;
    //cout << "InnerNode::removeChild: " << "keyIndex is " << keyIdx << endl;
    // TODO
	delete childrens[childIdx];
    //cout << "InnerNode::removeChild:  modify begin" << endl;
	memcpy(keys + keyIdx, keys + keyIdx + 1, sizeof(Key) * (nKeys - keyIdx - 1));
    //cout << "InnerNode::removeChild: key modify completed" << endl;
	memcpy(childrens + childIdx, childrens + childIdx + 1, sizeof(Node *) * (nChild - childIdx - 1));
    //cout << "InnerNode::removeChild: children modify completed" << endl;
...
KeyNode LeafNode::split() {
	...
	KeyValue kv_temp;
	for(uint64_t i = 0; i < 2 * LEAF_DEGREE - 1; i ++){
		for(uint64_t j = 0; j < 2 * LEAF_DEGREE - 1 - i; j ++){
			if(kv[j].k > kv[j + 1].k){
				kv_temp = kv[j];
				kv[j] = kv[j + 1];
				kv[j + 1] = kv_temp;
	...
	LeafNode *leafNode = new LeafNode(tree);
    // TODO
	uint64_t splitKey = findSplitKey();
	leafNode->n = LEAF_DEGREE;
	leafNode->prev = this;
	for(uint64_t i = 0; i < leafNode->bitmapSize; i++){
